...
Servers set the \mintinline{http}|Access-Control-Allow-Origin| header, and if matched with the origin, a browser allows the application to view and process its response. The header must be in format \mintinline{http}/Access-Control-Allow-Origin: <origin> | */.
...
If the widget changes, the \mintinline{text}{StatefulWidget} is used.
...
% ^^ TODO trochu vylepšit?
% Widget, Element, RenderObject trees -- v designu popsané
% https://www.youtube.com/watch?v=996ZgFRENMs&ab_channel=Flutter How Flutter renders Widgets - YouTube
